// ─────────────────────────────────────────────────────────────────────────────
// SEED CATALOG — ONE canonical source for the meal catalog the seed writes.
//
// The server `Meal` table is the FK parent for `TrayItem.mealId` (and
// `UserPlan.mealId`). The CLIENT posts dish ids from DISH_LIBRARY (679 live
// dishes) on first-load tray seed, suggestions, and add-to-tray — so the dev
// DB must contain every id the client can reference, or the write dies on
// `TrayItem_mealId_fkey`. This module converges the seed onto ONE catalog:
//   CURATED_DISHES — the hand-curated core (subset of the library, plus a
//                    few legacy dishes like dal-makhani that client offline
//                    fallback suggestions still reference)
//   DISH_LIBRARY   — the client's full live catalog (re-exported)
// `catalogMealIds()` is the union the seed upserts. Pure data: importing
// this module NEVER touches the DB (seed.ts owns the prisma side effects).
// Guarded by tests/seedCatalogCoverage.test.ts (Λ6.1 one canonical source).
// ─────────────────────────────────────────────────────────────────────────────
import { DISH_LIBRARY } from '../../meal/constants/dishLibrary';

export type CuratedDish = {
  id: string;
  name: string;
  icon: string;
  region: string;
...
export const CURATED_DISHES: CuratedDish[] = [
  ...
  {
    id: 'dal-tadka-central', name: 'Dal Tadka', icon: '🥣', region: 'north',
    category: ['lunch', 'dinner'], type: 'veg', tags: ['dal', 'lentils', 'everyday', 'comfort'],
    variants: [
      { id: 'dal-tadka-roti', name: 'Dal Tadka with Roti', addOn: 'with roti', mealContext: 'dinner' },
      { id: 'dal-tadka-rice', name: 'Dal Tadka with Rice', addOn: 'with rice', mealContext: 'lunch' },
      { id: 'dal-tadka-bafla', name: 'Dal Tadka with Dal Bafla', addOn: 'with dal bafla', mealContext: 'lunch' },
    ],
  },
  {
...
/** Every meal id the seed guarantees to exist in the `Meal` table. */
export function catalogMealIds(): Set<string> {
  const ids = new Set<string>();
  for (const d of CURATED_DISHES) ids.add(d.id);
  for (const d of DISH_LIBRARY) ids.add(d.id);
  return ids;
}

/** Every meal id the CLIENT can natively reference (client catalogue). */
export function libraryMealIds(): Set<string> {
  return new Set(DISH_LIBRARY.map((d) => d.id));
}
