...
const PantryPulse: React.FC = () => {
    ...
    const buildShareListMessage = (lang: ShareLanguage, _selectedSlots: string[]) => {
        ...
        let lines = `🛒 *${copy.pantryTitle}*\n`;
        lines += `📍 *Region:* ${user?.region || ''}\n`;
        lines += `📅 *For:* ${viewMode === 'household' ? "Household meals" : subView === 'tomorrow' ? "Tomorrow's meals" : "This week's meals"}\n`;
        lines += `✅ *In Kitchen:* ${checked.length} items\n`;
        lines += `❌ *Needed:* ${unchecked.length} items\n`;
        lines += `🍽️ *Total Meals:* ${viewMode === 'household' ? householdMeals.length : subView === 'tomorrow' ? tomorrowMeals.length : weekMeals.length + tomorrowMeals.length}\n\n`;
        ...
        for (const cat of ['produce', 'proteins', 'dairy', 'grains', 'spices', 'pantry', 'breads']) {
            ...
            catItems.forEach(i => {
                ...
                if (i.sources.length > 0) {
                    lines += ` — for ${i.sources.slice(0, 2).join(', ')}${i.sources.length > 2 ? ` +${i.sources.length - 2} more` : ''}`;
                }
                lines += '\n';
            });
        }
        ...
        for (const [slot, slotMeals] of Object.entries(groupedBySlot)) {
            lines += `\n☀️ *${slot.toUpperCase()}*\n`;
            slotMeals.forEach(m => {
                lines += `  • ${m.name}${m.variant ? ` (${m.variant})` : ''}\n`;
            });
        }
    ...
    return (
        <div className="min-h-screen bg-white pb-32 animate-in fade-in duration-500 ">
            ...
            <header className="px-4 pt-6 pb-5">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 bg-[#25D366] text-white px-4 py-2.5 rounded-2xl text-[11px] font-black uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-[#25D366]/30"
                        >
                            <Share2 size={14} />
                            Share
                        </button>
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[13px] font-bold text-emerald-600">✅ {checkedCount} in kitchen</span>
                    <span className="text-gray-300">·</span>
                    <span className="text-[13px] font-bold text-orange-600">❌ {uncheckedCount} to buy</span>
                </div>

                <div className="mt-5 bg-gray-50/80 rounded-2xl flex p-0.5 border border-gray-100">
                    {(['meals', 'household'] as const).map(v => {
                        if (v === 'household' && (!householdId || !household)) return null;
                        return (
                        <button
                            key={v}
                        ...
                        >
                            {v === 'meals' ? '📋 Meals' : '👥 Household'}
                        </button>
                        );
                    })}
                </div>
                {/* Sub-toggle: Tomorrow / This Week (only when Meals selected) */}
                {viewMode === 'meals' && (
                    <div className="mt-2 flex">
                        <div className="bg-gray-50/80 backdrop-blur-sm rounded-xl flex p-0.5 border border-gray-100/50 w-full">
                            {(['tomorrow', 'week'] as const).map(v => (
                            <button
                                key={v}
                                onClick={() => setSubView(v)}
                                className={`flex-1 py-2 rounded-lg text-[10px] font-bold tracking-widest transition-all duration-200 ${
                                    subView === v
                                        ? 'bg-white text-[#FF385C] shadow-sm border border-[#FF385C]/10'
                                        : 'text-gray-500 hover:text-gray-600 active:scale-95'
                                }`}
                            >
                                {v === 'tomorrow' ? 'Tomorrow' : 'This Week'}
                            </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                        {(['Breakfast', 'Lunch', 'Snacks', 'Dinner'] as const).map(slot => {
                            ...
                            return meals.length > 0 ? meals.map((m, i) => (
                                <div key={`${slot}-${i}`} className="flex flex-col items-center gap-1.5 flex-shrink-0">
                                    <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-gray-100 shadow-sm bg-gray-50">
                                        <DishImage name={m.name} slot={m.slot} size="full" className="w-full h-full object-cover" />
                                    </div>
                                    <p className="text-[10px] font-bold text-gray-800 leading-tight max-w-[72px] text-center truncate">{m.name}</p>
                                    <span className="text-[10px] font-bold text-[#FF385C] uppercase tracking-wider">{slot}</span>
                                </div>
                            )) : null;
                        })}
                    </div>
            ...
            {viewMode === 'household' && householdMeals.length > 0 && (
                <div className="px-4 mb-5">
                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-3">
                        🏠 Roommates ({householdMeals.length} meals)
                    </p>
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                        {(() => {
                            ...
                            return Object.entries(byMember).map(([name, meals]) => (
                                <div key={name} className="flex-shrink-0 bg-gray-50 rounded-xl p-3 border border-gray-100 min-w-[140px]">
                                    <p className="text-[10px] font-black text-[#FF385C] uppercase tracking-widest mb-2">{name}</p>
                                    {meals.slice(0, 5).map((m: any, i: number) => (
                                        <p key={i} className="text-[10px] font-bold text-gray-800 truncate">{m.name}</p>
                                    ))}
                                    {meals.length > 5 && <p className="text-[10px] text-gray-500 mt-1">+{meals.length - 5} more</p>}
                                </div>
                            ));
                        })()}
                    </div>
                ...
                <div key={group.category} className="mb-4">
                    <div className="sticky top-0 z-10 bg-white px-4 pt-3 pb-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">{group.label}</span>
                    ...
                    <div className="px-4 grid grid-cols-2 gap-2">
                        {group.items.map(item => {
                            ...
                            return (
                                <div
                                    key={item.id}
                                    className={`flex items-center gap-2 p-2.5 rounded-[16px] border-2 transition-all ${checked ? 'border-green-200 bg-green-50' : 'border-gray-100 bg-white'}`}
                                >
                                    <button
                                        onClick={() => setChecked(item.name, !checked)}
                                        className={`w-8 h-8 rounded-xl border-2 flex items-center justify-center transition-all flex-shrink-0 ${checked ? 'bg-green-500 border-green-500 text-white' : 'border-gray-200'}`}
                                    >
                                        {checked && <Check size={16} />}
                                    </button>
                                    <div className="flex-1 min-w-0">
                                        <span className={`font-bold text-xs block truncate ${checked ? 'line-through text-gray-500' : 'text-gray-800'}`}>
                                            {item.name}
                                        </span>
                                        ...
                                        {!checked && mealSources.length > 0 && (
                                            <span className="text-[10px] text-orange-500 block mt-0.5 font-medium truncate">
            ...
            {unmatchedManual.length > 0 && (
                <div className="px-4 mb-4">
                    <div className="flex items-center gap-2 mt-4 mb-2">
                        <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">Other Items</span>
                        <span className="text-[10px] font-black text-gray-200">{unmatchedManual.length} items</span>
                    </div>
                    <div className="space-y-1.5">
                        {unmatchedManual.map(item => {
                            ...
                            return (
                                <div
                                    key={item.id}
                                    className={`flex items-center gap-3 p-2.5 rounded-xl border transition-all ${checked ? 'border-green-200 bg-green-50' : 'border-gray-100 bg-white'}`}
                                >
                                    <button
                                        onClick={() => toggleItem(item.id)}
                                        className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 ${checked ? 'bg-green-500 border-green-500' : 'border-gray-300'}`}
                                    >
                                        {checked && <Check size={14} className="text-white" />}
                                    </button>
                                    <span className={`flex-1 text-xs font-medium truncate ${checked ? 'line-through text-gray-500' : 'text-gray-600'}`}>{item.name}</span>
