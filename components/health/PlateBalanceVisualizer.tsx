    ...
    <div className="p-5 rounded-2xl border-2 border-gray-200 bg-white">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black uppercase tracking-widest text-gray-500">Meal Balance</span>
        </div>
        ...
        {filteredCategories.map(([key, value]) => {
          ...
          return (
            <div key={key} className="flex items-center gap-2">
              <span className="text-sm font-bold text-gray-500 w-20 shrink-0">{categoryLabels[key] || key}</span>
              ...
              <span className="text-sm font-bold text-gray-400 w-4 text-right">{value}</span>
            </div>
          );
        })}
      </div>
      ...
      {filteredSuggestions.length > 0 && (
        <div className="mt-3 p-3 rounded-xl bg-amber-50 border border-amber-100">
          <p className="text-sm font-black uppercase tracking-widest text-amber-700 mb-1">Suggestions</p>
          {filteredSuggestions.map((s, i) => (
            <p key={i} className="text-sm text-amber-800 leading-relaxed">• {s}</p>
          ))}
        </div>
      )}
    </div>
