...
// Now uses pairing engine for dish-aware side/beverage pairings.
// ─────────────────────────────────────────────────────────────────────────────
...
export function dishToMeal(dish: Dish, variant?: DishVariant): Meal {
  ...
  // Use pairing engine FIRST — it's authoritative for sides/beverages
  const pairing = computePairingForDish(dish);

  // Only compute sideOptions from variants/accompaniments for non-standalone dishes
  ...
  if (!isStandalone) {
    ...
    // Standalone dishes (beverage, dessert, bread, side) — no savory sides
  ...
  // For standalone dishes, pairing.sides is authoritative (usually empty)
  // For other dishes, merge variant sides with pairing sides
