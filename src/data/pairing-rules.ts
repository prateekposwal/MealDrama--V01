// ─────────────────────────────────────────────────────────────────────────────
// Pairing Rules — Deterministic, culturally accurate dish-pairing engine
// Schema-validated lookup table for Indian meal combinations
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Curated pairing rules — ordered by priority.
 * First matching rule wins (specific → general).
 * 
 * Rules can match by:
 * - dishId (exact) → highest priority
 * - dishName (exact or partial) → high priority
 * - styles (array match) → medium priority
 * - regions (array match) → fallback
 * 
 * Each rule caps at: 2 sides, 1 condiment, 1 beverage, 1 dessert
 */
export const PAIRING_RULES: PairingRule[] = [
  // ─── Beverages (standalone — culturally relevant accompaniments only) ──────
  ...
  // ─── Breads (standalone or with beverage) ─────────────────────────────────
  ...
  // ─── Style-based fallback rules (catch-all for unmatched dishes) ──────────
  ...
  // ─── Region-based fallback (lowest priority) ──────────────────────────────
...
/** Validate all rules at module load time */
export function validatePairingRules(): PairingRule[] {
  return PAIRING_RULES.map((rule, i) => {
    if (!rule.dishId && !rule.dishName && !rule.styles && !rule.regions) {
      throw new Error(`PairingRule[${i}] must have at least one match key: dishId, dishName, styles, or regions`);
    }
    if (rule.sides.length > 2) {
      throw new Error(`PairingRule[${i}] sides array exceeds max of 2`);
    }
    if (rule.condiments.length > 1) {
      throw new Error(`PairingRule[${i}] condiments array exceeds max of 1`);
    }
    if (rule.priority && !['high', 'medium', 'low'].includes(rule.priority)) {
      throw new Error(`PairingRule[${i}] invalid priority: ${rule.priority}`);
    }
    return rule;
  });
}
